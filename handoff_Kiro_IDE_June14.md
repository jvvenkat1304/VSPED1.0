...
| July 17, 2026 | All changes committed + pushed to GitHub | Commit 86623b4: 57 files changed, 1445 insertions, 1103 deletions. Covers: 6 runtime bug fixes (auth/routing), 98/104 SonarQube issues resolved, 8 complexity refactors, 30 catch blocks with logging, missing notifications implemented, MCP config + bug tracker + steering files. Pushed to origin/main. Backend edge functions need redeployment to take effect. |
| July 17, 2026 | UI fix: SafeAreaView for status bar + nav button overlap + OTP autofill | Fixed 3 UI issues: (1) Content crashing into notification bar — added SafeAreaProvider to root layout + useSafeAreaInsets in auth screens (dynamic paddingTop replaces fixed 80px); (2) Bottom tab bar hidden on 3-button Android phones — SafeAreaProvider enables Expo Router Tabs native safe area handling (no extra padding on gesture phones); (3) OTP auto-read filling only last digit on Samsung/OnePlus — handleChange now spreads multi-digit input from index 0 regardless of which box triggered. Bug tracker fully rewritten with detailed won't-fix justifications. Created docs/reset-database.sql for fresh E2E testing. |
